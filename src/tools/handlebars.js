...
let rowCount = 1;
Handlebars.registerHelper('rowCount', () => {
  rowCount += 1;
  return rowCount;
});
...
const compileHeaderPartial = (param = {}) =>
  new Promise(async (resolve, reject) => {
    try {
      ...
      const headerStyleFile = await ReadFile(
        path.resolve(templateFolderPath, 'styles/header.scss'),
        'utf8'
      );
      ...
      //   attributes: ['companyName', 'companyRegistrationNo']
      // });
      // if (!user && userId) {
...
const compileFooterPartial = () =>
  new Promise(async (resolve, reject) => {
    try {
      const footerStyleFile = await ReadFile(
        path.resolve(templateFolderPath, 'styles/footer.scss'),
        'utf8'
      );
...
const preCompilePartials = () =>
  new Promise(async (resolve, reject) => {
    try {
      const bootstrapFile = await ReadFile(
        path.resolve(templateFolderPath, 'styles/bootstrap-utilities.css'),
        'utf8'
      );
      const commonFile = await ReadFile(
        path.resolve(templateFolderPath, 'styles/common.css'),
        'utf8'
      );
